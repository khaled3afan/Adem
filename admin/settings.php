...
///// GOOGLE CODE ///////////////////////////////////////////////////////////
    case "google-code" :
        if(isset($_POST['googleCode']) & !empty($_POST['googleCode'])){
            $return = array();
            $googleCode = strip_tags($_POST['googleCode'],"<script>,</script>");
            $script = "<script>"; 
            if(substr($googleCode,0,8) == $script){
                $updateCode = $settingsDB->DB_Query("UPDATE settings SET sett_value='$googleCode' WHERE sid=4");
                    if(isset($updateCode)){
                        $return['status'] = 1;
